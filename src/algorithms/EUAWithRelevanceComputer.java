 ...
 * Time : 13:36
 */
public final class EUAWithRelevanceComputer extends AComputer<ApproximatedTransitionSystem> {
    ...
    public EUAWithRelevanceComputer(Machine machine, LinkedHashSet<AbstractState> A, IRelevanceCheckingFunction relevanceCheckingFunction) {
    ...
    private void step3() {
                ...
                for (Event e : new DefaultEventsOrderingFunction().apply(machine.getEvents())) {
                    if (Delta.contains(new AbstractTransition(q, e, q_))) {
                        ...
                        if (z3.checkSAT() == SATISFIABLE) {
                            ...
                            ConcreteState c_ = new ConcreteState("c_" + q_.getName(), model.getTarget());
                            if (!C.contains(c_)) {
                                c_.setName("c_" + q_.getName() + "_" + C.size());
                            } else {
                                c_.setName(new ArrayList<>(C).get(new ArrayList<>(C).indexOf(c_)).getName());
                            }
        ...
        Tuple<LinkedHashSet<AbstractState>, LinkedHashSet<AbstractTransition>> reachablePart = new ReachableAbstractPartComputer(C0, DeltaC, Alpha).compute().getResult();
        System.out.println(Delta.stream().filter(abstractTransition -> !reachablePart.getSecond().contains(abstractTransition)).collect(Collectors.toList()));
    }
