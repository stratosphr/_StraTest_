 ...
 * Time : 13:36
 */
public final class NewEUAComputerNoWhile extends AComputer<ApproximatedTransitionSystem> {
    ...
    public NewEUAComputerNoWhile(Machine machine, LinkedHashSet<AbstractState> A) {
    ...
    protected void postCompute() {
        ...
        /*LinkedHashSet<AbstractState> preQ0 = new LinkedHashSet<>();
        LinkedHashSet<AbstractState> preRQ = new LinkedHashSet<>();
        ...
        preRQ.addAll(preQ0);
        while (!preRQ.isEmpty()) {
            AbstractState q = preRQ.iterator().next();
            preRQ.remove(q);
            Q.add(q);
            for (Event e : machine.getEvents()) {
                for (AbstractState q_ : A) {
