...
import algorithms.heuristics.IGuidingRelevanceFunction;
 ...
 * Time : 13:36
 */
public final class EUAWithRelevanceGuidedComputer extends AComputer<ApproximatedTransitionSystem> {
    ...
    private final Z3 z3;
    private final IGuidingRelevanceFunction relevanceCheckingFunction;

    public EUAWithRelevanceGuidedComputer(Machine machine, LinkedHashSet<AbstractState> A, IGuidingRelevanceFunction relevanceCheckingFunction) {
        ...
        while (!RCS.isEmpty()) {
            ...
            for (AbstractState q_ : new DefaultAbstractStatesOrderingFunction().apply(new Tuple<>(q, A))) {
                for (Event e : new DefaultEventsOrderingFunction().apply(machine.getEvents())) {
                    if (Delta.contains(new AbstractTransition(q, e, q_))) {
                        List<ABoolExpr> discriminatingExpressions = relevanceCheckingFunction.apply(c);
                        //for (ABoolExpr discriminatingExpression : discriminatingExpressions) {
                        Status status;
                        z3.setCode(new And(
                                machine.getInvariant(),
                                ...
                                new Or(discriminatingExpressions.toArray(new ABoolExpr[discriminatingExpressions.size()])),
                        ...
                        if (status == SATISFIABLE) {
                            ...
                            DeltaC.add(new ConcreteTransition(c, e, c_));
                                /*if (new ReachableAbstractPartComputer(C0, DeltaC, Alpha).compute().getResult().getSecond().containsAll(Delta)) {
                                    ...
                                    System.out.println(c_.getMapping().get(new IntVariable("CoffeeLeft")));
                                }*/
                        }
                        //}
                    }
